...
function client() {
  ...
  function customPredict(routeStops, cb, units) {
    ...
    var queryData = routeStops.map((item) => {
      ...
      if (agencyData.routes[routeTag] == undefined) {
        console.log("routeTag '" + routeTag + "' not found");
        return null;
      } else {
        routeStopTags = agencyData.routes[routeTag].stops;
      }
      return stopTags
