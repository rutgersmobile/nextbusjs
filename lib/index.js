...
function client() {
   ...
   * @param {[{routeTag: string, stop: string}]} routeStops
   * @param {function(err,data)} cb
   * @param {string} units
   */
  function customPredict(routeStops, cb, units) {
    if (!isAgencyCached) {
      cb({ name: "nocache", message: "no agency cache" }, null);
      return;
    }
    ...
    var queryData = routeStops.map((item) => {
      var routeTag = item.routeTag;
      var stop = item.stop;
      var stopItem, isTitle, stopTags;

      if (agencyData.stops[stop] !== undefined) {
        stopItem = agencyData.stops[stop];
        stopTags = [stopItem];
      } else if (agencyData.stopsByTitle[stop] !== undefined) {
        stopItem = agencyData.stopsByTitle[stop];
        stopTags = stopItem.tags;
      } else {
    ...
    // flatten array
    queryData = queryData.reduce((acc, val) => { acc + val }, []);

    var str = queryData.reduce((acc, val) => {
      return acc + "&stops=" + val.routeTag + "|" + val.direction + "|" + val.stopTag;
    }, "");

    query("predictionsForMultiStops", str, (err, response) => {
    });
  }
